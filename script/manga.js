...
if (isNaN(startPage) || isNaN(endPage) || startPage < 1 || endPage < startPage) {
    console.error('Invalid page range. Usage: node fetch-anilist-manga.js [start-end]');
    console.error('Example: node fetch-anilist-manga.js 1-200');
    process.exit(1);
}

const perPage = 50;
const pagesToFetch = Array.from({length: endPage - startPage + 1}, (_, i) => startPage + i);
const outputFile = `data/manga-${startPage}-${endPage}.json`;

async function sleep(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
}

async function fetchJapaneseManga(page) {
    const query = `
...
async function main() {
    ...
    for (let i = 0; i < pagesToFetch.length && shouldContinue; i++) {
        ...
        try {
            const data = await fetchJapaneseManga(page);
